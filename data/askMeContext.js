...
Positioning: Data Scientist specializing in causal inference, predictive modeling, and applied AI systems, with additional depth across analytics engineering, BI dashboard architecture, MLOps deployment, and business analysis. I build end-to-end pipelines from raw data through validated statistical models and deployed AI applications. Track record: a reproducible causal study of 40,000 synthetic customers, a 30% gain in reporting reliability, and production-style ML systems deployed on Kubernetes.
...
• Results post-fix: ARIMA beat XGBoost and LightGBM on every one of the five series. A Diebold-Mariano test (Harvey-Leybourne-Newbold corrected) reported p<0.05 favoring ARIMA in 27 of 40 series/model/horizon comparisons, including all ten at one quarter ahead, with fewer below that threshold at longer horizons. No multiple-comparison adjustment across these 40 tests is documented in this portfolio; do not interpret the count as a family-wide error guarantee. Pooling all five series into one global model helped the ML side but didn't close the gap; a nested Optuna hyperparameter search helped in only 4 of 10 cases.
• SHAP: the single most-recent lag carried 47–59% of reported aggregate SHAP importance across ten tree-model/series combinations. This does not mean 47–59% of each prediction. These attributions do not establish causal effects, explain ARIMA, or rule out training and tuning limitations as causes of the performance gap.
• Prediction intervals: ARIMA's native 95% prediction intervals achieved 98–100% empirical coverage, while the ML models' conformal intervals under-covered at 75–83%.
• Verdict: for this problem (univariate, ~140-quarter macro series, direct-horizon forecasting), classical ARIMA is the stronger model. The takeaway isn't "machine learning is bad"; model complexity has to match data size. The one lever that could plausibly change the result is genuinely new information (an exogenous macro regressor), not further feature engineering on the same short series.`,
	},
	{
		...
		text: `--- PROJECT: FanHouse Membership Impact Study (2026) ---
Independent synthetic portfolio study, not real company work. No randomized experiment was run in this version. Matching followed by DiD is one design, not independent methods.
Population, timing, and causal target: The synthetic panel contains 40,000 customers and 573,352 transactions from January 1, 2024 through December 31, 2025. Program launch is October 1, 2024: 274 pre-period days and 457 post-period days. There are 12,408 adopters and 27,592 non-adopters. The matched analysis retains 12,153 adopter-control pairs, about 97.94% of adopters. Its target is the full post-launch-window contrast for retained adopters under their observed adoption timing. It is not a randomized offer ITT or a uniform duration-since-adoption effect.
Outcome accounting: The primary outcome is net product payments per customer per day, after discounts and redeemed reward coins. Membership fees are tracked separately. The secondary outcome is product value at modeled pre-discount prices. Neither is contribution profit. Issued but unredeemed rewards, costs, and fee revenue recognition need separate accounting before a financial rollout decision. Physical units and willingness to pay are not measured by pre-discount product value.
Matching and estimation: A logistic propensity model uses pre-program covariates. Greedy one-to-one matching without replacement uses a default caliper of 0.03. Difference-in-differences is then computed within the matched pairs: (member post − member pre) − (control post − control pre). The pre-spend standardized mean difference falls from 0.3961 to 0.00134. The largest absolute matched model-covariate SMD is approximately 0.0152. These are observed-balance diagnostics, not proof of exchangeability. The net-payment estimate is −$0.0102538/day, paired SE $0.0066222, p = 0.1216, 95% CI [−$0.0232345, +$0.0027268]. The pre-discount estimate is +$0.1367276/day, 95% CI [+$0.1225555, +$0.1508997].
Event study and identification limits: Monthly specifications use pair-clustered covariance. Only 49.5% of adopters join in the launch month. Launch-aligned paths therefore mix cohorts and exposure durations; they should not be read as cohort-specific treatment dynamics. The overall joint pretrend diagnostic has raw p = 0.0165 and Holm-adjusted p = 0.1154 across seven diagnostics. Report both: adjusted non-rejection does not establish parallel trends or practical equivalence. Identification still requires assumptions about parallel counterfactual trends, selection, anticipation, and interference. Matching cannot remove unobserved confounding by itself.
Sensitivity and channel findings: At caliper 0.10 the net-payment estimate becomes nominally significant (p = 0.0467), compared with p = 0.1216 at the default. Statistical significance is therefore not stable across all matching specifications. The in-store net-payment contrast is −$0.01210/day (nominal p = 0.0131); online is +$0.00184/day (p = 0.7123). These channel results do not establish a profitable overall program. The interactive dashboard includes four monthly outcomes and an additive trend-violation sensitivity control.
Validation against known simulated outcomes: The shared generator constructs coupled factual and no-benefit worlds and verifies the factual reconstruction before producing counterfactual output. The known full-window net-payment ATT is −$0.0112774/day. The estimate differs by about +$0.0010236/day and contains that truth in its interval. All six outcome/channel comparisons contain their coupled truth, but they are correlated outcomes in one world, not a coverage study. A separate Monte Carlo exercise uses 20 configured-benefit and 20 zero-benefit worlds, each with 4,000 customers and matching refitted. Both scenarios cover truth in 20/20 intervals; each Wilson 95% interval is approximately 83.9% to 100%. Zero of 20 null worlds rejects at 5%. This is too small to establish precise coverage or Type I error, and it is not full-sample validation.
Reproducibility and decision: The pipeline regenerates data, runs 14 regression tests, executes the analysis and assumption audit, builds the dashboard and preview, and validates artifact consistency. The manifest records completion, package versions, source/data hashes, and phase logs. Separate JavaScript checks exercise dashboard bindings and sensitivity updates. The implementation checks support this simulation. They do not establish real-world identification. The next organizational study should randomize an appropriate offer or rollout and measure contribution profit before a launch recommendation.
Do not repeat obsolete claims about 50,000 customers, 325 randomized stores, an 8x inflation, a $42 gap, $32–33 TOT, $5 ITT, or $11.6M–$19.9M rollout revenue. Do not recommend launching on this evidence.
Links: https://github.com/Sreekaran1704/fanhouse-casestudy-membership- ; https://sreekaran1704.github.io/fanhouse-casestudy-membership-/ ; https://github.com/Sreekaran1704/fanhouse-casestudy-membership-/blob/main/CASE_STUDY.md`,
	},
	{
		id: 'marketPulse',
...
Limitation: results describe the collected Adzuna and RemoteOK postings, not the entire hiring market. Listing counts are not confirmed hires or necessarily distinct vacancies. Source coverage, duplicates, missing salary fields, and collection timing can affect comparisons; do not claim those issues have been eliminated without supporting evidence.
...
• Secured the remote HTTP transport with bearer-token authentication and a per-IP sliding-window rate limiter, and layered a Groq-powered LLM summary on top of the computed statistics to translate raw hiring trends into a plain-English narrative based on those statistics. Generated summaries can contain errors and should be checked against the underlying results.
...
Scope: academic prototype. Clinical validation and real-world use are not documented in this portfolio. Describe the follow-up rules as prototype examples, not deployed care recommendations or evidence of patient benefit.
